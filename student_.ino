...
String LFSR(String message) {
    ...
    for (size_t i = 0; i < message.length(); i++) {
        char c = message[i] ^ (lfsr_state & 0xFF);
        encryptedMessage += c;
        bool newBit = ((lfsr_state >> 15) ^ (lfsr_state >> 13) ^ (lfsr_state >> 12) ^ (lfsr_state >> 10)) & 1;
        lfsr_state = (lfsr_state << 1) | newBit;
    }
    return encryptedMessage;
}
